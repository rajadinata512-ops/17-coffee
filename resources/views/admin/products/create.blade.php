...
@section('title', 'Tambah Menu - 17 Coffee')

@section('content')
<div class="max-w-4xl">
  <div class="mb-6">
    <h1 class="text-2xl font-black text-[#2b160b]">Tambah Menu Baru</h1>
    <p class="text-sm text-gray-500 mt-1">Tambahkan menu kopi atau non kopi.</p>
  </div>
  ...
  <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl p-6 shadow-sm border border-[#ead9c9]">
    @csrf
    ...
    <div class="grid md:grid-cols-2 gap-5">
      ...
      <div>
        <label class="block text-sm font-bold text-[#2b160b] mb-2">Stok Awal <span class="text-red-500">*</span></label>
      ...
      <div>
        ...
        <select name="status" required
            class="w-full rounded-xl border border-[#ead9c9] px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#c28a3d]">
          <option value="active" {{ old('status', 'active') === 'active' ? 'selected' : '' }}>Aktif, tampil di menu</option>
      ...
      <div class="md:col-span-2">
        ...
        <textarea name="description" rows="4"
             class="w-full rounded-xl border border-[#ead9c9] px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#c28a3d]"
             placeholder="Ceritakan keunikan minuman ini...">{{ old('description') }}</textarea>
      </div>
      ...
      <div class="md:col-span-2">
        ...
        <p class="text-xs text-gray-500 mt-2">Format: JPG, JPEG, PNG, WEBP, JFIF. Maksimal 4 MB.</p>
      </div>
    </div>
